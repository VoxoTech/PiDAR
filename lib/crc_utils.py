def split_last_byte(data):
    return data[:-1], data[-1]

def shift_bytes(data, shift=1):
    return data[-shift:] + data[:-shift]

def check_CRC8(data, crc=None):
    '''CRC check: length is 1 Byte, obtained from the verification of all the previous data except itself'''
    
    # CRC-8 Table
    CRC_TABLE = []
    
    if crc is None:
        data, crc = split_last_byte(data)
    
    calculated_crc = 0
    for byte in data:
        if not 0 <= byte <= 255:
            raise ValueError(f"Invalid byte value: {byte}")
        calculated_crc = CRC_TABLE[(calculated_crc ^ byte) & 0xff]

    return calculated_crc == crc


if __name__ == "__main__":
    ## correct: starting with 0x54, 0x2c
    # byte_string = b'T,\x10\x0eS+\x04\x01\xec\x13\x01\xea\x1a\x01\xea\x1e\x01\xea&\x01\xe91\x01\xe8)\x01\xeb,\x01\xe9-\x01\xeb4\x01\xe88\x01\xe5N\x01\xd9\xe4.W\x0b\xd3'
    
    # BUG: ending with 0x54, 0x2c
    byte_string = b'\x10\x0eS+\x04\x01\xec\x13\x01\xea\x1a\x01\xea\x1e\x01\xea&\x01\xe91\x01\xe8)\x01\xeb,\x01\xe9-\x01\xeb4\x01\xe88\x01\xe5N\x01\xd9\xe4.W\x0b\xd3T,' 
    byte_string = shift_bytes(byte_string, shift=2)

    # data, crc = split_last_byte(byte_string)
    # print("valid package:", check_CRC8(data, crc))

    print("valid package:", check_CRC8(byte_string))
